#include "ColorsExercice3.h";

void ColorsExercice3::framebuffer_size_callback(GLFWwindow* window, int width, int height)
{
	glViewport(0, 0, width, height);
}
void ColorsExercice3::processInput(GLFWwindow *window)
{
	if (glfwGetKey(window, GLFW_KEY_ESCAPE) == GLFW_PRESS)
		glfwSetWindowShouldClose(window, true);
}

int ColorsExercice3::run()
{
	glfwInit();
	...
	vertexShader.setVec3("objectColor", glm::vec3(1.0f, 0.5f, 0.31f));//objectColor
	vertexShader.setVec3("lightColor", glm::vec3(1.0f, 1.0f, 1.0f));
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		vertexShader.setVec3("lightPos", lightPos);
		vertexShader.setVec3("viewPos", cam.getPosition());
